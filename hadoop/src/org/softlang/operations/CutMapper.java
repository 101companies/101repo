...
public class CutMapper {
	...
	/**
	 * Mapper that reads Employee objects and writes the queried Employee's
	 * salary.
	 */
	public static class EmployeeMapper extends
			Mapper<Text, Employee, Text, Employee> {
	...
	/**
	 * Mapper that reads Department objects and writes all Employee's salary for
	 * the queried Department.
	 */
	public static class DepartmentMapper extends
			Mapper<Text, Department, Text, Department> {
		private static String name;
		private static boolean allValues;
		...
		protected void map(Text key, Department value, Context context)
				throws IOException, InterruptedException {
			if (allValues || value.getName().toString().equals(name)) {
