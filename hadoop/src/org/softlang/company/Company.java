...
public class Company implements WritableComparable<Company> {
	...
	/**
	 * Write (say, serialize) this Company object to disk, along with all its corresponding Departments and Employees.
	 */
	public void writeObject(String filename) {
		...
		try {
			...
			SequenceFile.Writer companyWriter = SequenceFile.createWriter(fs, conf, companyFile, Text.class, Company.class); 
			companyWriter.append(this.getName(), this);
			companyWriter.close();
			
			SequenceFile.Writer departmentWriter = SequenceFile.createWriter(fs, conf, departmentFile, Text.class, Department.class); 
			Set<Employee> employeeSet = new HashSet<Employee>();
			for(Department d : this.getDepts()){
				departmentWriter.append(d.getName(), d);
				d.getEmployeesRecursive(employeeSet); 	//in case the same employee is assigned to different departments
			}
			departmentWriter.close();

			SequenceFile.Writer employeeWriter = SequenceFile.createWriter(fs, conf, employeeFile, Text.class, Employee.class); 
	...
	public static Company readObject(String filename) {
		...
		try {
			fs = companyFile.getFileSystem(conf);
			SequenceFile.Reader companyReader = new SequenceFile.Reader(fs, companyFile, conf); 
